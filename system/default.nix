...
}@args:
{
  ...
  nix.settings = {
    ...
    # cache.nixos.org is named again on purpose. NixOS adds to this list, but
    # nix-darwin replaces it, so leaving it out takes the public cache away
    # from Hizack. A NixOS machine ends up listing it twice, which costs one
    # redundant query and nothing else.
    #
